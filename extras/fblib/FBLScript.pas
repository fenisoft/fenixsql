...
begin
  ...
  if (PScript = nil) or (PScript^ = #0) then
    Exit;
  PTail := PScript;
  InQuote := False;
  InComment := False;
  ...
  repeat
    ...
    while True do
    begin
      while (InQuote and not (PTail^ in [QuoteChar, #0])) or not
        (PTail^ in Separators + [#0, '''', '"']) do
      begin
        if PTail^ = #10 then
          Inc(FLineNumber);
        PTail := CStrNextChar(PTail);
  ...
  try
    ...
    begin
      ...
      if (Tokens.Strings[0] = 'CREATE') or (Tokens.Strings[0] = 'RECREATE') or
        (Tokens.Strings[0] = 'DECLARE') or (Tokens.Strings[0] = 'ALTER') or
        (Tokens.Strings[0] = 'DROP') then
        Result := stDDL;
