...
type
  ...
  public
    ...
    #)}
    procedure ExecSQL;
    {Move an (re)open cursor to first record }
    procedure First;
    ...
    {Return the current count of record fetched}
    property RecordCount: integer read GetFetchCount;
  ...
  published
    ...
    property AutoFetchFirst: boolean
      read FAutoFetchFirst write FAutoFetchfirst default True;
    {occurs when cursor is closed}
    property OnCloseCursor: TNotifyEvent read FOnCloseCursor write FOnCloseCursor;
    {occurs when current statement is closed}
    property OnUnPrepare: TNotifyEvent read FOnUnPrepare write FOnUnPrepare;
  end;
...
begin
  if AParamIdx > (ParamCount - 1) then
    FBLError(E_QR_PAR_OUT_OFF_RANGE, [AParamIdx]);
end;
...
begin
  if AFieldIdx > (FieldCount - 1) then
    FBLError(E_QR_FIELD_OUT_OFF_RANGE, [AFieldIdx]);
end;
...
begin
  for i := 0 to ParamCount - 1 do
    if not FParamAssigned[i] then
      FBLError(E_QR_PARAM_NOT_SET, [i]);
end;
...
begin
  {$IFDEF D9P}
  SqlText  :=   WideStringToString(FSQL.Text)  ;
  {$ELSE}
  SqlText :=   FSQL.Text;
  {$ENDIF}
  CheckTransaction;
  ...
  {$ENDIF}
    ...
    if isc_dsql_prepare(@Status_Vector, TRHandle, @Fstmt, 0,
      PAnsiChar(SqlText), Transaction.Database.SQLDialect, nil) <> 0 then
      FBLShowError(@Status_vector);
    ...
    case FQueryType of
      qtGetSegment, qtPutSegment,
      qtStartTransaction:
        FBLError(E_QR_NOTPERMITTED, [Sql]);
...
procedure TFBLDsql.OnSqlChange(Sender: TObject);
begin
  if Fstmt <> nil then
    UnPrepare;
end;
    ...
    case QueryType of
      ...
      begin
        ...
        if (Status_vector[0] = 1) and (Status_vector[1] <> 0) then
          FBLShowError(@Status_vector);
        if isc_dsql_set_cursor_name(@Status_vector, @FStmt,
          PAnsiChar(FCursor), 0) <> 0 then
          FBLShowError(@Status_vector);
        FEOF := False;
        FBOF := True;
        FCursorOpen := True;
        FFetchCount := 0;
        if FAutoFetchFirst then
          Next;
      end;
      qtExecProcedure:
      begin
        if isc_dsql_execute2(@Status_Vector, TRHandle,
          @FStmt, Transaction.DataBase.SQLDialect, FPXSQLDA_IN,
          FPXSQLDA_OUT) <> 0 then
          FBLShowError(@Status_vector);
    ...
    try
      if (FStmt <> nil) and (QueryType = qtSelect) and FCursorOpen then
        if isc_dsql_free_statement(@Status_vector, @Fstmt, DSQL_close) <> 0 then
          FBLShowError(@Status_vector);
      if Assigned(FOnCloseCursor) then
        FOnCloseCursor(self);
    finally
      FCursorOpen := False;
      FEOF := True;
...
var
  Status_Vector: ISC_STATUS_VECTOR;
  Buffer: array[0..1048] of AnsiChar;
  info_char: AnsiChar;
begin
  {$IFDEF FBL_THREADSAFE}
  Lock;
  ...
  {$ENDIF}
    ...
    begin
      ...
      case QueryType of
        ...
        qtSelect: Result := isc_vax_integer(@Buffer[20], 4);
...
function TFBLDsql.GetPlan: string;
var
  Status_Vector: ISC_STATUS_VECTOR;
  Buffer: array[0..16384] of AnsiChar;
  Len: Integer;
  Info_char: AnsiChar;
  VResult: AnsiString;
begin
    ...
    case QueryType of
      ...
      begin
        ...
        isc_dsql_sql_info(@Status_vector, @FStmt, 1, //2
          @info_char, Short(SizeOf(Buffer)), Buffer);
        if (status_vector[0] = 1) and (status_vector[1] <> 0) then
          FBLShowError(@Status_vector);
        Len := isc_vax_integer(@buffer[1], 2);
        Setlength(VResult, Len);
        Move(Buffer[3], VResult[1], Len);
        {$IFDEF D9P}
        Result := UnicodeString(Trim(VResult));
        {$ELSE}
         Result := Trim(VResult);
        {$ENDIF}
      end;
...
begin
  ...
  Result := n and (FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlind^ = -1);
end;
...
var
  DataPtr: PAnsiChar;  // pointer to xsqlda.sqldata
    ...
    case FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqltype and (not 1) of
      ...
      begin
        if FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlsubtype = 1 then
          VResult := BlobFieldAsString(AFieldIdx)
        else
          VResult := '(Blob)';
      end;
      SQL_TEXT:
      ...
      SQL_LONG:
        if FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlscale = 0 then
          VResult := IntToStr(FieldAsLong(AFieldIdx))
        else
          VResult := FloatToStr(FieldAsDouble(AFieldIdx));
      SQL_DOUBLE:
        VResult := FloatToStr(FieldAsDouble(AFieldIdx));
      SQL_FLOAT,
      SQL_D_FLOAT:
        VResult := FloatToStrF(FieldAsFloat(AFieldIdx), ffGeneral, 7, 8);
      SQL_TYPE_DATE:
        VResult := DateToStr(FieldAsDatetime(AFieldIdx));
      SQL_TYPE_TIME:
        VResult := TimeToStr(FieldAsDatetime(AFieldIdx));
      SQL_DATE:
        VResult := DateTimeToStr(FieldAsDatetime(AFieldIdx));
      SQL_INT64:
        if FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlscale = 0 then
          VResult := IntToStr(FieldAsInt64(AFieldIdx))
        else
          VResult := FloatToStr(FieldAsDouble(AFieldIdx));
      else
    ...
    case FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqltype and (not 1) of
      ...
      begin
        if FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlscale = 0 then
          ...
          Result := Long(Trunc(GetNumericValue(
            int64(PShort(FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqldata)^),
      ...
      begin
        if FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlscale = 0 then
          ...
          Result := Long(Trunc(GetNumericValue(
            int64(PLong(FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqldata)^),
            FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlscale)));
      end;
      SQL_INT64:
        if FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlscale = 0 then
          Result := Long(PInt64(FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqldata)^)
        else
          Result := Long(Trunc(GetNumericValue(PInt64(FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqldata)^,
            FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqlscale)));
        else
      FBLError(E_QR_FIELD_CONV, [AFieldIdx]);
    end;
    ...
    case FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqltype and (not 1) of
      ...
      begin
        try
          ...
          on E: EConvertError do
            FBLError(E_QR_FIELD_TIME_CONV, [AFieldIdx]);
        end;
      end;
      ...
      begin
        try
          ...
          on E: EConvertError do
            FBLError(E_QR_FIELD_DATE_CONV, [AFieldIdx]);
        end;
      end;
      ...
      begin
        try
          isc_decode_timestamp(PISC_TIMESTAMP(FPXSQLDA_OUT^.sqlvar[AFieldIdx].sqldata),
            @tm);
          Result := EncodeDatetime(word(tm.tm_year + 1900),
            word(tm.tm_mon + 1), word(tm.tm_mday), word(tm.tm_hour),
            word(tm.tm_min), word(tm.tm_sec), 0);
        except
          on E: EConvertError do
            FBLError(E_QR_FIELD_TIMESTAMP_CONV, [AFieldIdx]);
        end;
      end;
      else
        FBLError(E_QR_FIELD_CONV, [AFieldIdx]);
...
begin
  {$IFDEF D9P}
  VValue :=  WideStringToString(AValue);
  {$ELSE}
  VValue := AValue;
  {$ENDIF}
  Pvchar := nil;
  ...
  {$ENDIF}
    ...
    case FPXSQLDA_IN^.sqlvar[AParamIdx].sqltype and (not 1) of
      ...
      begin
        if len_value < FPXSQLDA_IN^.sqlvar[AParamIdx].sqllen then
          FillChar(FPXSQLDA_IN^.sqlvar[AParamIdx].sqldata^,
            FPXSQLDA_IN^.sqlvar[AParamIdx].sqllen, ' ');
        if len_value > 0 then
          Move(VValue[1], FPXSQLDA_IN^.sqlvar[AParamIdx].sqldata^, len_value);
      end;
      SQL_VARYING:
      begin
        ...
        try
          ...
          begin
            ...
            Move(VValue[1], Pvchar^.str, len_value);
          end;
      ...
      begin
        try
          ...
          on E: Exception do
            FBLError(E_QR_PARAM_TIME_CONV, [AParamIdx]);
        end;
      end;
      SQL_TYPE_DATE:
      begin
        try
          ...
          on E: Exception do
            FBLError(E_QR_PARAM_DATE_CONV, [AParamIdx]);
        end;
      end;
      SQL_TIMESTAMP:
      begin
      ...
      begin
        try
          ...
          on E: Exception do
            FBLError(E_QR_PARAM_CONV, [AParamIdx]);
        end;
      end;
      else
        FBLError(E_QR_PARAM_TYPE, [AParamIdx]);
...
//          else
      ...
      SQL_LONG:
        ...
        begin
          {$IFDEF D9P}
          VResult := AnsiString(IntToStr(_ParamAsLong(AParamIdx)));
          {$ELSE}
          VResult :=  IntToStr(_ParamAsLong(AParamIdx));
          {$ENDIF}
        end
        else
        begin
          {$IFDEF D9P}
          VResult := AnsiString(FloatToStr(_ParamAsDouble(AParamIdx)));
          {$ELSE}
          VResult := FloatToStr(_ParamAsDouble(AParamIdx));
          {$ENDIF}
        end;
      SQL_DOUBLE:
        {$IFDEF D9P}
      ...
      SQL_FLOAT,
      SQL_D_FLOAT:
        VResult := FloatToStr(_ParamAsFloat(AParamIdx));
      SQL_TYPE_DATE:
        VResult := DateToStr(_ParamAsDateTime(AParamIdx));
      SQL_TYPE_TIME:
        VResult := TimeToStr(_ParamAsDateTime(AParamIdx));
      SQL_DATE:
        VResult := DateTimeToStr(_ParamAsDateTime(AParamIdx));
      SQL_INT64:
        if FPXSQLDA_IN^.sqlvar[AParamIdx].sqlscale = 0 then
          VResult := IntToStr(_ParamAsInt64(AParamIdx))
        else
          VResult := FloatToStr(_ParamAsDouble(AParamIdx));
      else
  ...
  {$ENDIF}
    case FPXSQLDA_IN^.sqlvar[AParamIdx].sqltype and (not 1) of
      ...
      begin
        ...
        tm.tm_min := m;
        tm.tm_sec := s;
        isc_encode_timestamp(@tm,
          PISC_TIMESTAMP(FPXSQLDA_IN^.sqlvar[AParamIdx].sqldata));
      end;
      else
        FBLError(E_QR_PARAM_TYPE, [AParamIdx]);
    end;
...
var
  ...
  Buffer: array [0..BLOB_SEGMENT_LEN] of AnsiChar;
  //Buffer: array [0..BLOB_SEGMENT_LEN] of Byte;
  TotLen: Integer;
begin
  Blob_Handle := nil;
  CheckFieldIdx(AFieldIdx);
