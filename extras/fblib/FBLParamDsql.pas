    ...
    ':':
      begin
        ...
        while (j <= Length(Line)) and (
          ...
          inc(j);
        mParamName := UpperCase(Copy(Line, p + 1, j - p - 1));
        Line[p] := '?';
        Delete(Line, p + 1, j - p - 1);
        d := d + j - p - 1;
      end
    end;
    FParamMap.Add(mParamName);
    if mParamName <> '?' then
    	FParamNames.Add(mParamName);
    p := FScanner.GetNextParameterPosition;
  end;
  ...
  Line := FParamSQL.Text;
    ...
    begin
        ...
        But now, the result of this processing is
  		...
  		ParamID := 0;
      mParamName := format(ParamNameTemplate, [ParamId]);
      ...
      while FParamMap.IndexOf(mParamName) >= 0 do
      begin
        inc(ParamID);
      	mParamName := Format(ParamNameTemplate, [ParamId]);
      end;
			FParamMap[j] := mParamName;
      FParamNames.Add(mParamName);
