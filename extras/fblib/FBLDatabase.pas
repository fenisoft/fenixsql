...
type
  ...
  private
    ...
    procedure SendBeforeDisconnectEvent;
    procedure SendAfterDisconnectEvent;
  public
  ...
  published
    ...
    connection without opening a dataset.
    Set Connected to @False to close a database connection.
    An application can check Connected to determine
    the current status of a database connection.
    If Connected is @True, the database connection is active;
    if @False, then the connection is inactive.@html(<br>)
    see also @LINK(Connect) @LINK(Disconnect)
    }
...
begin
  ...
  {$ENDIF}
    ...
    FAttachObjs.Free;
    if Assigned(FBackoutCount) then
      FBackoutCount.Free;
    if Assigned(FDeleteCount) then
      FDeleteCount.Free;
    if Assigned(FExpungeCount) then
      FExpungeCount.Free;
    if Assigned(FInsertCount) then
      FInsertCount.Free;
    if Assigned(FPurgeCount) then
      FPurgeCount.Free;
    if Assigned(FReadIdxCount) then
      FReadIdxCount.Free;
    if Assigned(FReadSeqCount) then
      FReadSeqCount.Free;
    if Assigned(FUpdateCount) then
      FUpdateCount.Free;
...
//------------------------------------------------------------------------------
function TFBLDatabase.GetAttachObjectCount: integer;
begin
  Result := FAttachObjs.Count;
end;
...
function TFBLDatabase.GetConnected: boolean;
begin
  {$IFDEF FBL_THREADSAFE}
  Lock;
  try
...
begin
  if DBFile = '' then
    FBLError(E_DB_NONAME);
  case FProtocol of
    ptLocal:
      FConnectString := DBFile;
    ptTcpIp:
      if FHost = '' then
...
begin
  ...
  Params := 'USER ' + QuotedStr(AUser) + ' PASSWORD ' + QuotedStr(APassword) +
...
procedure TFBLDatabase.SetConnected(const Value: boolean);
begin
  ...
  begin
    SendBeforeDisconnectEvent;
    DoDisconnect;
    FDBHandle := nil;
    if Assigned(FOnDisconnect) then
      FOnDisconnect(self);
    SendAfterDisconnectEvent;
  end;
end;
